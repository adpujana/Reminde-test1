...
# jika semua terbuang (misal CSV ganti total), fallback ke semua unit_cols
#if not st.session_state.saved_monitored_cols:
    #st.session_state.saved_monitored_cols = unit_cols.copy()
    
# jangan auto-pilih semua kalau kosong
# biarkan kosong sampai user memilih
pass
...
#        options=unit_cols,
#        default=st.session_state.saved_monitored_cols,
#        disabled=monitor_all   # ⬅️ nonaktifkan jika monitor_all ON
#    )
...
# commit nilai hanya saat submit
#if submitted:
...
# alarm muncul 4 menit sebelum event
alarm_trigger_time = event_target - pd.Timedelta(minutes=4)
